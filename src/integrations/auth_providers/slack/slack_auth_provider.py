...
from .slack_bindings import SlackBinding
from .slack_credentials import SlackAppCredentials, SlackUserCredentials


class SlackAuthProvider(AuthProvider[SlackAppCredentials, SlackUserCredentials]):
    """Auth entry point for Slack integrations."""

    app_credentials_class = SlackAppCredentials
    user_credentials_class = SlackUserCredentials

    oauth2: OAuth2Flow[SlackAppCredentials, SlackUserCredentials] = flow(
        lambda provider: provider._build_oauth2_flow()
    )

    def default_bindings(self) -> Mapping[AuthProviderKey, SlackBinding]:
        return {AuthProviderKey.SLACK: SlackBinding()}

    def _build_oauth2_flow(
        self,
    ) -> OAuth2Flow[SlackAppCredentials, SlackUserCredentials]:
        ...
        return OAuth2Flow(
            ...
            token_class=SlackUserCredentials,
        )
